...
template <typename... T>
std::tuple<pq::writer<T>...> make_message_writers(boost::program_options::variables_map const& options, boost::mp11::mp_identity<std::tuple<pq::writer<T>...>>) {
   ...
   return {pq::writer<T>{make_parquet_sink<T>(path), properties}...};
}
} // namespace
...
std::ptrdiff_t parquet_writer::operator()(nasdaq::moldudp64::v1_0::seqn_t /*seqn*/, tc::byte_t const* begin, tc::byte_t const* end) {
    ...
    std::visit([this](auto const& message) {
       auto& writer = std::get<pq::writer<std::decay_t<decltype(message)>>>(writers_);
